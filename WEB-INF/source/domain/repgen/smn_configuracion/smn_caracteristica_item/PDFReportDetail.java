...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cit_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_item_rf_name");
			if (d != null)
			{
			name  = "b_item";
			label = "${lbl:b_item}";
		...
		d = rs.getString("cit_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("cit_descripcion_tecnica");
			if (d != null)
			{
			name  = "b_descripcion_tecnica";
			label = "${lbl:b_descripcion_tecnica}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
