package domain.repgen.smn_configuracion.smn_sub_division;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("sdi_descripcion");
			if (d != null)
			{
			name  = "b_description";
			label = "${lbl:b_description}";
		...
		d = rs.getString("smn_almacen_rf_name");
			if (d != null)
			{
			name  = "b_stores";
			label = "${lbl:b_stores}";
		...
		d = rs.getString("sdi_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("sdi_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_division_id_name");
			if (d != null)
			{
			name  = "b_division";
			label = "${lbl:b_division}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_division_id");
			if (d != null)
			{
			name  = "b_division";
			label = "${lbl:b_division}";
