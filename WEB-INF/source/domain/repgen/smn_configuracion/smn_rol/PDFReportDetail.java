package domain.repgen.smn_configuracion.smn_rol;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_sucursales_rf");
			if (d != null)
			{
			name  = "b_subsidiaries";
			label = "${lbl:b_subsidiaries}";
		...
		d = rs.getString("smn_usuarios_rf_name");
			if (d != null)
			{
			name  = "b_users";
			label = "${lbl:b_users}";
		...
		d = rs.getString("smn_corporaciones_rf");
			if (d != null)
			{
			name  = "b_corporations";
			label = "${lbl:b_corporations}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rol_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_usuarios_rf");
			if (d != null)
			{
			name  = "b_users";
			label = "${lbl:b_users}";
		...
		d = rs.getString("rol_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date_greather_than";
			label = "${lbl:b_store_date_greather_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_sucursales_rf_name");
			if (d != null)
			{
			name  = "b_subsidiaries";
			label = "${lbl:b_subsidiaries}";
		...
		d = rs.getString("smn_entidades_rf");
			if (d != null)
			{
			name  = "b_entities";
			label = "${lbl:b_entities}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_corporaciones_rf_name");
			if (d != null)
			{
			name  = "b_corporations";
			label = "${lbl:b_corporations}";
		...
		d = rs.getString("smn_entidades_rf_name");
			if (d != null)
			{
			name  = "b_entities";
			label = "${lbl:b_entities}";
