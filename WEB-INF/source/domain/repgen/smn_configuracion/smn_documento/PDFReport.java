...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_tipo_documento_id_name");
			if (d != null)
			{
			name  = "b_document_type";
			label = "${lbl:b_document_type}";
		...
		d = rs.getString("smn_tipo_documento_id");
			if (d != null)
			{
			name  = "b_document_type";
			label = "${lbl:b_document_type}";
		...
		d = rs.getString("doc_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_transaccion_general_rf_name");
			if (d != null)
			{
			name  = "b_general_transaction";
			label = "${lbl:b_general_transaction}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_transaccion_general_rf");
			if (d != null)
			{
			name  = "b_general_transaction";
			label = "${lbl:b_general_transaction}";
		...
		d = rs.getString("doc_fecha_registro_ini");
			if (d != null)
			{
